...
```{r}
n_funcionarios_ativos <- function(type) {
  ...
  return(list(df = df, smry = smry))

}

get_summary_formatted <- function(tp, calc) {
  n_funcionarios_ativos(type = tp)$smry %>%
    pull({{calc}}) %>% 
    round() %>% 
    format(big.mark = ".", decimal.mark = ",")
}

n_servidores_ativos <- get_summary_formatted("servidor", mean_funcionarios_ano)
n_membros_ativos <- get_summary_formatted("membro", mean_funcionarios_ano)

gasto_medio_servidores_ativos <- get_summary_formatted("servidor", gasto_medio_por_funcionario)
gasto_medio_membros_ativos <- get_summary_formatted("membro", gasto_medio_por_funcionario)

versus_salario_minimo <-
  n_funcionarios_ativos(type = "membro")$smry %$% 
  round(gasto_medio_por_funcionario / 2080.91)

pagtos_over_100k <- 
  incomes %>% 
  filter(aid %in% c("tjpb", "trt13", "mppb")) %>% 
  select(active, income_total) %>% 
  arrange(desc(income_total)) %>% 
...
```{r}
pib_2018_pb <- 
  httr::GET("https://www.ibge.gov.br/explica/pib.php") %>% 
  httr::content(as = "parsed") %>% 
  rvest::html_table() %>% 
  pluck(1) %>%
  filter(X1 == "Paraíba") %$% 
  as.numeric(str_remove_all(X2, "\\.")) * 1e6

gasto_jus_pb <- incomes %>% filter(year == 2018) %$% sum(income_total)

perc_jus_pib_pb <- scales::percent(gasto_jus_pb / pib_2018_pb)
```

Gastos com remunerações de quatro órgãos do sistema de Justiça na Paraíba equivalem a **`r perc_jus_pib_pb`** do PIB estadual. Ou seja, a cada R\$ 100 reais produzidos em todo o estado, R\$ 2 são usados no pagamento de salários, gratificações, benefícios e uma longa lista de auxílios e indenizações a juízes, procuradores e outros funcionários públicos do TJ-PB, do MP-PB, do TRT-13 e do Tribunal Regional Eleitoral da Paraíba (TRE-PB). 

```{r}
gasto_medio_mensal <- function(aid) {
  incomes %>% 
  filter(aid == aid) %>% 
  group_by(year, month) %>% 
  summarise(gasto_mensal = sum(income_total), .groups = "drop") %>% 
  group_by(year) %>% 
  summarise(gasto_mensal_medio = mean(gasto_mensal), .groups = "drop") %$%
  round(mean(gasto_mensal_medio) / 1e6)
}

gasto_mensal_tjpb <- gasto_medio_mensal(aid = "tjpb")
gasto_mensal_trt13 <- gasto_medio_mensal(aid = "tjpb")
gasto_mensal_mppb <- gasto_medio_mensal(aid = "mppb")
gasto_mensal_trepb <- gasto_medio_mensal(aid = "trepb")

funcionarios_mes <- function(aid) {
  incomes %>%
  filter(aid == aid) %>%
  group_by(year, month) %>%
  summarise(n_funcionarios_mes_media = n(), .groups = "drop") %$% 
  mean(n_funcionarios_mes_media) %>% 
  format(decimal.mark = ",", big.mark = ".")
}

n_funcionarios_tjpb <- funcionarios_mes(aid = "tjpb")
n_funcionarios_trt13 <- funcionarios_mes(aid = "trt13")
n_funcionarios_mppb <- funcionarios_mes(aid = "mppb")
n_funcionarios_trepb <- funcionarios_mes(aid = "trepb")
```

TJ-PB gasta desde 2018 em média **R\$`r gasto_mensal_tjpb`** milhões por mês com pagamentos para cerca de **`r n_funcionarios_tjpb`** membros e servidores. O TRT-13 gasta em torno de **R\$`r gasto_mensal_trt13`** milhões mensais cerca de **`r n_funcionarios_trt13`** membros e servidores, mas apresenta picos significativos em novembro e dezembro em função de benefícios. O MP-PB gasta em torno de **R\$`r gasto_mensal_mppb`** milhões com seus aproximados **`r n_funcionarios_mppb`** funcionários e o TRE, mais de **R\$`r gasto_mensal_trepb`** milhões com aproximadamente de **`r n_funcionarios_trepb`** funcionários.
...
mean_income_total_type_year %>% 
  ggplot(aes(x = type, y = media_pagtos / 1e3, fill = as.factor(year))) +
...
* **Benefícios**: [**TO DO**: preencher tabela](https://docs.google.com/spreadsheets/d/1DpApfCT2qAdOACwYHYvAyh2XB7E3lB4hNbUBfCcq5Dc/edit#gid=571028202)

* **Indenizações**: [**TO DO**: são pagamentos referentes à auxilio alimentação, férias, transporte, auxílio creche, auxílio saúde, auxílio natalidade, auxílio moradia, ajuda de custo, licença compensatória e outras compesações por direito adquirido.](https://docs.google.com/spreadsheets/d/1DpApfCT2qAdOACwYHYvAyh2XB7E3lB4hNbUBfCcq5Dc/edit#gid=571028202)
...
diarias <- incomes %>% 
  ...
  summarise(
    ...
    .groups = "drop"
  ) %>% #ggplot(aes(x = as.factor(qt_diarias))) + geom_histogram( stat="count") + facet_wrap(~type, scales = "free")
  ...
  summarise(
    ...
    .groups = "drop"
  ) %>% #arrange(aid, type, year) %>% print(n = Inf) 
  group_by(aid, type) %>% 
...
diarias_fmt <- function(.aid) {
  ...
  mutate(qt_diarias = qt_diarias_servidor + qt_diarias_membro,
         ...
         across(where(is.numeric), ~ format(.x, big.mark = ".", decimal.mark = ",")))
}
...
Dados do DadosJusBr mostram que, entre 2018 e 2020, na média, o TJ-PB realizou pagamentos a **`r diarias_tjpb$qt_diarias`** pessoas inativas e ativas na folha de pagamento da Corte. Somente **`r diarias_tjpb$qt_diarias_membro`** eram magistrados. Apesar de serem somente **`r diarias_tjpb$pc_qt_diarias_membro`**, os magistrados embolsaram em média **`r diarias_tjpb$pc_vl_diarias_membro`** do valor anual das diárias pagas pelo Tribunal a toda sua folha de pagamento. Mais de **R\$ `r diarias_tjpb$vl_diarias_total_membro` mil/ano** foram para os magistrados em rubricas na qual não incidem o teto constitucional.
...
maior_diaria_paga <- incomes %>%
  ...
  mutate(
    perks_daily_fmt = format(perks_daily, big.mark = ".", decimal.mark = ","),
    income_total_fmt = format(income_total, big.mark = ".", decimal.mark = ","),
    income_mean_year = format(income_mean_year, big.mark = ".", decimal.mark = ","),
    aid = toupper(aid) %>% gsub("^(.+)(.{2}$)", "\\1-\\2", .)
  )
```


A maior diária paga foi a um servidor do **`r maior_diaria_paga$perks_daily_fmt[1]`**. Um **`r maior_diaria_paga$role[1]`** desembolsou 
R\$ **`r maior_diaria_paga$perks_daily_fmt[1]`** em **`r month.name[maior_diaria_paga$month[1]]`** de **`r maior_diaria_paga$year[1]`**. A segunda maior foi a um **`r maior_diaria_paga$role[2]`** do **`r maior_diaria_paga$perks_daily_fmt[2]`**, que em **`r month.name[maior_diaria_paga$month[2]]`** de **`r maior_diaria_paga$year[2]`** recebeu sozinho R\$ **`r maior_diaria_paga$perks_daily_fmt[2]`**. A diária aumentou substantivamente pagamentos tanto do analista como do magistrado. O analista viu seu salário subir da média anual de R\$ **`r maior_diaria_paga$income_mean_year[1]`** para R\$ **`r maior_diaria_paga$income_total_fmt[1]`** e o corregedor de R\$ **`r maior_diaria_paga$income_mean_year[2]`** para R\$ **`r maior_diaria_paga$income_total_fmt[2]`**.